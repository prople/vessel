    ...
    pub fn parse(&self) -> Result<Config, ConfigError> {
        let config_toml = {
            let config_builder: Result<Config, ConfigError> =
                Builder::new(from_file(self.conf_file.to_owned())).fetch()?.parse(use_toml);
...
mod tests {
    ...
    fn test_parse_config() {
        ...
        let (dbpath, cfname)= config_db.identity.get_common();
        ...
        assert!(config_db_opts.get_set_error_if_missing());
    }
}